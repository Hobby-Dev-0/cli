...
# import DeulexClient
from DeulexClient import DeulexClient ,setbot , get_bot , get_self, vr ,Adminsettings, vcbot
from assistant import ALL_AST
from adminss import ALL_ADMINN
from utilss import ALL_UTILS
from ownerr import ALL_OWN
from DeulexClient.plugins import ALL_INB
...
async def start_bot():
    ...
    for moduloo in ALL_INB:
        imported_module = importlib.import_module("DeulexClient.plugins." + moduloo)
        if hasattr(imported_module, "__MODULE__") and imported_module.__MODULE__:
            imported_module.__MODULE__ = imported_module.__MODULE__
        if hasattr(imported_module, "__MODULE__") and imported_module.__MODULE__:
            if not imported_module.__MODULE__.lower() in HELP_COMMANDO:
