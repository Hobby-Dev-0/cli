...
Msge={}
@DeulexClient.on_message(filters.command("help",".") & filters.user(Adminsettings))
async def h_lp(_ , message):
  booet= await setbot.get_me()
  res=await DeulexClient.get_inline_bot_results(booet.username, "hlpin")
...
async def in_h_lp(_ , inline_query):
    ...
    stosen = InputTextMessageContent(
        message_text=f"You are accessing help for **Deulex Userbot** \n __Deulex Client.__ \n\n ♕𝚃𝚘𝚝𝚊𝚕 𝙲𝚘𝚖𝚖𝚊𝚗𝚍𝚜♛= {𝚕𝚎𝚗𝚐𝚝}")
...
async def cbire(_ , cbq: CallbackQuery):
   ...
   if cdt == "b_k":
        ...
        await cbq.edit_message_caption(
                            caption=f"You are accessing help for **Deulex Userbot** \n __Everyone is a Deulex. Until the real Deulex arrives.__",
                            )
        await cbq.edit_message_reply_markup(reply_markup=InlineKeyboardMarkup(keyboard))
        return
   if cdt == "kloz":
